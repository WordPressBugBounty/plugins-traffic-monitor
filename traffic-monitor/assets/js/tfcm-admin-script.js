...
jQuery(document).ready(function ($) {
	 ...
	 *
	 * @param {string} type    The notice type (example: 'success', 'error').
	...
	// Only run filter-preserving column link behavior on the Traffic Monitor log table page
	if (typeof tfcmAdmin !== 'undefined' && tfcmAdmin.page === 'traffic-monitor') {
		...
		/**
		 * Handles dismissal of the old log export notice.
		 * Delete this on or after version 4.0.0
		 *
		 * When the user clicks the "dismiss" (×) button on the export notice,
		 * this sends an AJAX POST request to remove the associated option from the database.
		 * The notice will not reappear after being dismissed.
		 */
		$(document).on('click', '#tfcm-old-log-export-notice .notice-dismiss', function () {
			$.post(tfcmAdmin.ajax_url, {
				action: 'tfcm_dismiss_export_notice',
				nonce: tfcmAdmin.nonce,
			});
		});

		// Preserve current filter and search values when sorting columns
		const tfcmFilters = ['tfcm_user_role', 'tfcm_bot_name', 'tfcm_bot_category', 'tfcm_ad_platform', 's'];

		const currentValues = {};
		tfcmFilters.forEach(function (name) {
		...
		document.querySelectorAll('.wp-list-table th a').forEach(function (link) {
